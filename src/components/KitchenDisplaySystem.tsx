...
interface KitchenDisplaySystemProps {
  ...
  onToggleOrderItemComplete?: (orderId: string, itemId: string, isCompleted: boolean) => Promise<void>;
}
...
export const KitchenDisplaySystem: React.FC<KitchenDisplaySystemProps> = ({
  ...
  // 🔊 語音合成朗讀新訂單狀態與服務 (TTS Order Readout Service)
  ...
  const speak = (text: string) => {
    ...
    try {
      ...
      const zhVoice = voices.find(v => v.lang.includes('zh') || v.lang.includes('zho') || v.lang.includes('CHN'));
      if (zhVoice) {
  ...
  // Monitor for incoming new orders
  useEffect(() => {
    ...
    if (newOrders.length > 0) {
      ...
      if (ttsEnabled) {
        newOrders.forEach((o, index) => {
          let tableStr = o.tableNumber;
          if (!tableStr || tableStr.trim() === '' || tableStr.toLowerCase() === 'takeout' || tableStr.toLowerCase() === '外帶') {
            tableStr = '外帶';
          } else {
            tableStr = tableStr + '桌';
          }
          // Intentionally delay multiple messages slightly to prevent overlap cuts
          setTimeout(() => {
            speak(`收到新訂單，桌號 ${tableStr}`);
          }, index * 2600);
        });
      }

      // 🆙 Auto-scroll to top for immediate kitchen visibility of new orders
      if (autoScrollEnabled) {
        // Delay slightly so the new element rendering has completed in React virtual DOM
        setTimeout(() => {
          scrollToHeaderTop();
        }, 150);
      }
    }
  }, [orders, ttsEnabled, autoScrollEnabled]);
  ...
  const [isSimulatedMode, setIsSimulatedMode] = useState<boolean>(true); // default true for sandbox testing, can toggle
      ...
      <div className="xl:col-span-3 space-y-5">
        <div className="bg-[#161616] text-white rounded-xl p-5 border border-white/10 space-y-4" id="kds-header-main-box">
          ...
          <div className="flex flex-wrap items-center gap-2.5 text-xs text-white/60 pt-0.5">
            ...
            {/* 🔊 語音合成服務 Speech Synthesis Service Toggle */}
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (onToggleOrderItemComplete) {
                                  onToggleOrderItemComplete(order.id, it.id, !it.isCompleted);
                                }
                              }}
                              className={`shrink-0 h-10 px-4 rounded-xl text-xs font-black transition-all duration-200 shadow-sm flex items-center justify-center gap-1 border cursor-pointer ${
                                it.isCompleted
                                  ? 'bg-emerald-500 hover:bg-emerald-600 text-[#0F0F0F] border-emerald-400 shadow-emerald-500/10'
                                  : 'bg-[#1e1e1e] hover:bg-[#252525] text-emerald-400 hover:text-emerald-300 border-emerald-500/35 hover:border-emerald-500 shadow-black/20'
                              }`}
                              title={it.isCompleted ? "標示為未完成 (Mark as Pending)" : "標示為製作完成 (Mark as Completed)"}
                            >
                              {it.isCompleted ? (
                                <>
                        ...
                        <div className="flex items-center justify-between gap-3 flex-wrap">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (onToggleOrderItemComplete) {
                                  onToggleOrderItemComplete(quickViewOrder.id, it.id, !it.isCompleted);
                                }
                              }}
                              className={`h-10 px-4 rounded-xl text-xs font-black transition-all duration-200 shadow-sm flex items-center justify-center gap-1 border cursor-pointer ${
                                it.isCompleted
                                  ? 'bg-emerald-500 hover:bg-emerald-600 text-[#0F0F0F] border-emerald-400 shadow-emerald-500/10'
                                  : 'bg-[#1e1e1e] hover:bg-[#252525] text-emerald-400 hover:text-emerald-300 border-emerald-500/35 hover:border-emerald-500 shadow-black/20'
                              }`}
