...
export const ManagerDashboard: React.FC<ManagerDashboardProps> = ({
      ...
      const rows = sortedOrders.map((order) => {
        const itemSummaries = order.items.map((it) => {
          ...
          if (it.customization?.selectedAddOns && it.customization.selectedAddOns.length > 0) {
            const addOnsText = it.customization.selectedAddOns.map((addon: any) => `+${addon.name} x${addon.qty || 1}`).join(',');
  ...
  const cashierCalculatedTotals = useMemo(() => {
    ...
    const sub = cashierMergedOrders.reduce((sum, o) => {
      ...
      return sum + (o.items?.reduce((curSum: number, it: any) => curSum + it.price * it.qty, 0) || 0);
    }, 0);
  ...
  const handleCashierCheckoutSubmit = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn('⚠️ Firestore upload failed, continuing with local POS checkout flow gracefully:', err);
  ...
  const triggerEditMenuItemMode = (item: any) => {
    ...
    setItemRecipe(Array.isArray(item.recipe) ? [...item.recipe] : []);
    setNewRecipeIngId('');
    setNewRecipeAmount('1');
    setIsFormOpen(true);
  };
                                    ...
                                    <div key={it.id} className="pt-1.5 flex justify-between items-center text-zinc-300 font-sans">
                                      <div className="text-left space-y-0.5">
                                        <p className="font-bold text-white">
                                          {pName} <span className="font-mono text-[#E5B453] ml-1">x{it.qty || 0}</span>
                                        </p>
                                        {it.customization && (
                                          <span className="text-[10px] text-zinc-500 block">
                                            辣度: {['不辣', '微辣', '中辣', '泰式大辣'][it.customization.spiciness || 0]} · 
                                            甜度: {['無糖', '減糖', '標準', '多糖'][it.customization.sweetness || 0]}
                                      ...
                                      <div className="flex items-center space-x-2 shrink-0">
                                        ...
                                        <div className="flex items-center bg-black/40 rounded-lg p-0.5 border border-white/5">
                                          ...
                                          <span className="px-1 text-[10px] font-black text-white min-w-[12px] text-center">{it.qty || 0}</span>
                                          <button
                                            type="button"
                                            onClick={() => handleCombinedQtyChange(ord.id, it.id, 1)}
                                            className="p-1 hover:bg-white/5 rounded text-zinc-400 hover:text-white transition cursor-pointer"
                    ...
                    <table className="w-full text-xs text-zinc-300">
                      ...
                      <tbody className="divide-y divide-white/5">
                        {reservations.length === 0 ? (
                          <tr>
                            <td colSpan={7} className="p-6 text-center text-zinc-500 font-medium font-sans">
                    ...
                    <button
                      ...
                      onClick={() => {
                        setPrintConfirmData({
                          ...
                          ip: kitchenPrinter.ip || '10.0.0.124',
                          onConfirm: async () => {
                            if (onPrintTestPage) {
                              try {
                    ...
                    <button
                      ...
                      onClick={() => {
                        setPrintConfirmData({
                          ...
                          ip: billPrinter.ip || '10.0.0.124',
                          onConfirm: async () => {
                            if (onPrintTestPage) {
                              try {
                                const res = await onPrintTestPage();
                                if (res.success) {
                                  alert('✓ 🧾 前台收銀測試列印請求已成功發送！');
                                } else {
                                  alert(`⚠️ 列印失敗: ${res.error || '無法存取設備'}`);
                                }
                              } catch (err) {
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4">
              ...
              <div className="space-y-3.5 text-left">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3.5">
                  ...
                  <div className="space-y-1">
                    <span className="text-zinc-500 font-sans block text-[10px]">預訂時間 Time *</span>
                    <input type="time" required value={resTimeInput} onChange={(e) => setResTimeInput(e.target.value)} className="w-full bg-[#1e1e1e] border border-white/10 rounded px-2.5 py-1.5 text-white font-mono" />
                  </div>
                </div>
                ...
                <div className="space-y-1">
                  ...
                  <textarea value={resNotesInput} onChange={(e) => setResNotesInput(e.target.value)} placeholder="加不辣/嬰兒椅/需靠窗等需求" rows={3} className="w-full bg-[#1e1e1e] border border-white/10 rounded px-2.5 py-1.5 text-white resize-none" />
                </div>
              </div>
            </div>
            {/* Modal Fixed Footer */}
              ...
              <div className="bg-black/35 border border-white/5 p-4 rounded-xl space-y-3">
                ...
                <div className="flex justify-between items-center text-zinc-400">
                  ...
                  <span className="text-amber-300 font-bold bg-amber-500/10 px-2 py-0.5 rounded-md">
                    ...
                    {cashierPaymentMethod === 'linepay' && '📱 LINE Pay'}
