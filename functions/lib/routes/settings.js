...
function registerSettingsRoutes(app, ctx) {
    ...
    get('/settings/service-pause', async (_req, res) => {
        try {
            ...
            if (helpers_1.cachedServicePause && (nowMs - helpers_1.cachedServicePause.timestamp < helpers_1.CACHE_TTL_MS)) {
                return res.json(helpers_1.cachedServicePause.data);
            }
            const sysData = await getCachedSettings();
            const data = { servicePaused: sysData?.liveServicePaused || false };
            (0, helpers_1.setCachedServicePause)({ data, timestamp: nowMs });
            res.json(data);
        }
        catch (error) {
    ...
    post('/admin/clear-test-data', requireStaffAuth, async (req, res) => {
        ...
        try {
            ...
            if ((0, auth_1.hashPin)(pin) !== storedHash) {
    ...
    post('/settings/notifications', requireStaffAuth, async (req, res) => {
        try {
            ...
            if (lineEnabled !== undefined)
                updates.lineEnabled = Boolean(lineEnabled);
            if (lineAdminId !== undefined)
                updates.lineAdminId = String(lineAdminId).trim();
            if (typeof lineToken === 'string' && lineToken.trim()) {
                updates.lineToken = lineToken.trim();
            }
            if (gmailEnabled !== undefined)
                updates.gmailEnabled = Boolean(gmailEnabled);
            if (gmailUser !== undefined)
                updates.gmailUser = String(gmailUser).trim();
            if (typeof gmailAppPass === 'string' && gmailAppPass.trim()) {
                updates.gmailAppPass = gmailAppPass.replace(/\s+/g, '').trim();
            }
            await docRef.set(updates, { merge: true });
            (0, helpers_1.setCachedNotificationSettings)(null);
    ...
    post('/settings/notifications/test', requireStaffAuth, async (req, res) => {
        try {
            ...
            const result = await (0, notification_1.sendTestNotification)(channel, effectiveConfig);
            if (result.success) {
